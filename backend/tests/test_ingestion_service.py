...
class IngestionServiceTests(unittest.TestCase):
    ...
    def test_23_page_pdf_with_nul_artifacts_indexes_all_44_chunks(self):
        from app.services.nlp_service import preprocess_text as actual_preprocess_text

        # Generated bytes exercise real PDF inspection. Only extracted page text
        # is supplied separately because a PDF writer may sanitize NUL glyphs.
        with pymupdf.open() as document:
            for page_number in range(1, 24):
                document.new_page().insert_text((72, 72), f"Synthetic page {page_number}.")
            content = document.tobytes()
        pages = []
        for page_number in range(1, 24):
            words = ["evidence"] * (300 if page_number <= 21 else 120)
            if page_number == 5:
                words[10:15] = ["micro\x00scope", "evi\x00dence", "repro\x00ducible", "\u03b2", "\u2264"]
            pages.append({"page_number": page_number, "text": " ".join(words)})
        self.assertEqual(sum(page["text"].count("\x00") for page in pages), 3)

        with patch.object(ingestion, "extract_text_from_pdf_bytes", return_value=pages):
            with patch.object(ingestion, "preprocess_text", wraps=actual_preprocess_text) as preprocess:
                result = self.ingest(content=content)

        self.assertEqual(result["status"], "success")
        self.assertEqual(result["document"]["status"], "indexed")
        self.assertEqual(result["document"]["page_count"], 23)
        self.assertEqual(result["document"]["chunk_count"], 44)
        chunks = self.database.chunks[UUID(result["document"]["id"])]
        self.assertEqual(len(chunks), 44)
        self.assertEqual(preprocess.call_count, 44)
        self.assertTrue(all("\x00" not in call.args[0] for call in preprocess.call_args_list))
        for chunk in chunks:
            self.assertNotIn("\x00", chunk.chunk_text)
            self.assertNotIn("\x00", chunk.processed_text)
            self.assertEqual(chunk.token_count, len(chunk.processed_text.split()))
        affected = next(chunk for chunk in chunks if (chunk.page_number, chunk.chunk_number) == (5, 1))
        self.assertIn("microscope evidence reproducible \u03b2 \u2264", affected.chunk_text)
        self.assertIn("\u03b2", affected.processed_text)
        self.assertEqual(self.events, ["create_document", "upload", "processing", "insert_chunks", "index_build", "indexed", "index_publish"])

    def test_nul_free_scientific_text_keeps_existing_nlp_output(self):
        from app.services.nlp_service import preprocess_text as actual_preprocess_text

        paragraph = "\u03b2 cells show \u2264 42 observations. Researchers studied caf\u00e9 samples and microstructures."
        expected_tokens = actual_preprocess_text(paragraph)
        pages = [{"page_number": 1, "text": paragraph}]
        with patch.object(ingestion, "extract_text_from_pdf_bytes", return_value=pages):
            with patch.object(ingestion, "preprocess_text", wraps=actual_preprocess_text) as preprocess:
                result = self.ingest()
        preprocess.assert_called_once_with(paragraph)
        chunk = self.database.chunks[UUID(result["document"]["id"])][0]
        self.assertEqual(chunk.chunk_text, paragraph)
        self.assertEqual(chunk.processed_text, " ".join(expected_tokens))
        self.assertEqual(chunk.token_count, len(expected_tokens))
